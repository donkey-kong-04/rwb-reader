...
public class Step_PermissionSetOther extends Step {
	...
	public void getSheetIDS(PRWorkbook w) {
		...
		for(int i=index_start; i<=index_end; i++) {
			...
			if(isColumnHidden == false) {	
				...
				PRUtil.writeMsg("HIDDEN PERMISSIONSET " + header + " - "  + w.currentSheet.getSheetName(), Color.ORANGE, false);
		...
		if(type.trim().equals("")) {
	        ...
	        for(int i=index_start; i<=index_end; i++) {
				...
				if(isColumnHidden == false) {
					...
					XML_PermissionSet f = (XML_PermissionSet) w.getCorrectCorrectFile(XML_Profile.class, currentId);
					//System.out.println(index_start + " " + index_end);
					String value = w.currentRow.getCell(i).getStringCellValue().trim();
					if(type.equals("userPermissions")) {
